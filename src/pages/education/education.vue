          ...
          <div class="flex justify-center" v-if="AllArray">
            <div class="info_item xl:w-11/12 xl:min-h-98 xl:bg-cityBg xl:flex xl:flex-wrap xl:justify-start"
               v-if="AllArray.length > 0">
                  <div 
                     class="
                     item xl:w-1/4 xl:min-h-96 xl:border-r-2 xl:border-r-gray-500 px-4 xl:my-6
                     cards aos-init
                     " 
                     ...
                     data-aos-delay="100"
                     >
                     <div class="desc xl:min-h-60 relative">
                        <div class="info_name xl:text-xl xl:w-60 hover:cursor-pointer">
                           <a :href="'#' +'city_info' + index">{{city.name}}</a>
                        </div>
                        <div class="circle_info xl:flex xl:text-xs">
                           {{city.description}}
                        </div>
                        <div class="time absolute left-0 bottom-0 text-gray-500">
                           {{ moment(city.date_time * 1000).format("YYYY-MM-DD") }}
                        </div>
                      </div>
                     <div class="images" @click="ModalShow(city,index)">
                        <img class="w-72 h-44" :src="FILE_URL + 'images/' + city.Images[0].url" alt="">
                     </div>
               </div>
            </div>
          </div>
